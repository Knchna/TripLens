...
REGION_KEYWORDS = {
    "Kerala": ["kerala", "munnar", "alleppey", "wayanad", "kovalam", "thekkady", "kochi"],
    "Himachal Pradesh": ["himachal", "shimla", "manali", "kasol", "manikaran", "dalhousie", "dharamshala"],
    "Goa": ["goa"],
...
THEME_KEYWORDS = {
    ...
    "adventure": ["adventure", "trek", "rafting", "paragliding"],
    "heritage": ["heritage", "temple", "fort", "culture", "historic"],
    "pilgrimage": ["pilgrimage", "spiritual", "temple visit"],
    "hill-station": ["hill station", "mountains", "snow", "valley"],
    "honeymoon/romance": ["honeymoon", "romantic", "couple"],
    "family": ["family", "kids"],
}

PACE_KEYWORDS = {
    "Relaxed": ["relaxed", "leisure", "slow", "peaceful", "unrushed"],
    "Packed": ["active", "packed", "fast-paced", "full circuit", "express"],
}
...
def extract_preferences(prompt_text: str) -> dict:
    ...
    # Budget: range or single value, supporting ₹12,000 / 12000 / 12k formats
    range_match = re.search(
        r"(?:between|from)?\s*(?:₹|rs\.?)?\s*(\d{1,3}(?:,\d{3})*|\d+k)\s*(?:and|to|-)\s*(?:₹|rs\.?)?\s*(\d{1,3}(?:,\d{3})*|\d+k)",
        text,
    )
    if range_match:
        ...
        single_match = re.search(
            r"(?:under|below|budget|within|max)\s*(?:of)?\s*(?:₹|rs\.?)?\s*(\d{1,3}(?:,\d{3})*|\d+k)", text
        )
        if single_match:
            prefs["budget_max"] = _parse_amount(single_match.group(1))

    # Duration: "5 days", "5-day", "around a week"
    ...
    # Travelers count
    travelers_match = re.search(r"(\d+)\s*(?:people|travelers|travellers|persons|pax)", text)
    if travelers_match:
    ...
    # Start location — check common origin cities explicitly
    for city in ["delhi", "kochi", "chandigarh", "bangalore", "bengaluru", "mumbai", "chennai", "kolkata", "hyderabad"]:
        if city in text:
    ...
    # Interests (theme keywords) — can match multiple
    matched_interests = []
